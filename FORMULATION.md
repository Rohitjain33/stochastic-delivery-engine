...
$$ \mathcal{S} = \{\text{ON\_TIME}, \text{DELAYED}, \text{SEVERE}\} $$
...
Where $\gamma(X_i)$ is an explicit state-dependent penalty multiplier (e.g., $\gamma(\text{ON\_TIME}) = 1.0$, whereas $\gamma(\text{SEVERE}) = 1.8$).
